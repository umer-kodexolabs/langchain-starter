...
def main():
    ...
    for index, q in enumerate(predefined_questions, start=1):
        ...
        user_answers.append({"question": q, "answer": answer})

        follow_up_decision = input("Would you like follow-up questions? (yes/no): ").strip().lower()
        if follow_up_decision == "yes":
            followups = followup_tool.invoke({"question": q, "answer": answer})
            for line in followups.strip().split("\n"):
                if line.strip() == "":
                    continue
                print(f"\n{line}")
                follow_up_answer = input("Your answer: ")
                user_answers.append({"question": line, "answer": follow_up_answer})

    print("\nGenerating summary...")
    input_data = {"qna_list": user_answers}
    summary = summary_tool.invoke(input_data)
    print("\n--- Summary ---")
    print(summary)
